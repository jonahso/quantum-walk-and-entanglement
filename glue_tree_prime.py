...
def generate_glued_tree_hamiltonian_prime(depth: int, seed: int = None, degree=3, verbose: bool = True) -> np.ndarray:
    """
    ...
    if degree == 2:
        ...
        random.shuffle(tree0_leaves_shuffled) 
        random_cycle = [x for pair in zip(tree0_leaves_shuffled, tree1_leaves_shuffled) for x in pair]
        random_cycle.append(tree0_leaves_shuffled[0]) 

        for u, v in zip(random_cycle[:-1], random_cycle[1:]):
            H[u, v] = H[v, u] = 1
            gluing_pairs.append((u, v))
    else:
